...
def verificar_remedios():
    ...
    for doc in docs:
        ...
        if hora:
            ...
            # Adiciona o remédio programado à lista
            remédios_programados.append(f"- Remédio: {remedio} | Hora: {hora_formatada} | Data: {data_formatada}")
            
            # Verificando se a hora e a data atual coincidem com algum horário programado
            if data_formatada == data_atual and hora_formatada == hora_atual:
                remédios_a_tomar.append(remedio)
    
    # Mostra todos os remédios programados
    if remédios_programados:
        print("\nRemédios programados:")
        for remédio in remédios_programados:
            print(remédio)
    
    # Mensagem final sobre se é hora de tomar o remédio ou não
    if remédios_a_tomar:
        print(f"\nÉ hora de tomar o(s) remédio(s) agora: {', '.join(remédios_a_tomar)}")
    else:
